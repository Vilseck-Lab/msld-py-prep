...
#####################################################
def openbabel_convert(arg_inFile, arg_intype, arg_outFile, arg_outtype)->bool:
    """
    Converts a `.mol` file into a `.mol2` file using openbabel.
    """
    outstr = f'<openbabel_convert> Conversion of {arg_intype} file [{arg_intype}] to {arg_outtype} file [{arg_outFile}] '
    from openbabel import openbabel as OB
    
    converter = OB.OBConversion()
    omol = OB.OBMol()
    converter.SetInAndOutFormats(arg_intype, arg_outtype)
    converter.ReadFile(omol, arg_inFile)
    status = converter.WriteFile(omol, arg_outFile)

    if status:
        print(f'{outstr} succeeded.')
    else:
        print(f'{outdir} failed.')
        
    return 
#END


def readAtoms(rtffile):
    """ Read a given rtf and return atom names.
    Internally, read a rtf file to get atom names for each fragment 
...
def writeALF_Files(sysname, mcsout, outdir,cgenff, wd = '.', charmm=False, yml = True):
    """
    Write CHARMM input script and/or ALF YAML file
    that's needed by ALF.
    ...
    """
    fp=open(os.path.join(outdir,'nsubs'),'r')
    nsubs=[]
    ...
    for site in range(len(nsubs)):
        for sub in range(nsubs[site]):
            Sname=f"site{site+1}_sub{sub+1}"
            Snames.append(Sname)
    ...
    atomnames=[]
    for fname in Snames:
        atomnames.append(readAtoms(rtffile = f'{outdir}/{fname}_pres.rtf'))
    ...
    if charmm:
        ...
        fcharmm = os.path.join(wd, f"{sysname}.inp")
        print(f"<msld_wrt> Writing CHARMM file for ALF in {fcharmm}")
    ...
    if yaml:
        ...
        # write ALF yaml file
        fyalf = os.path.join(wd, f"{sysname}.yml")
        print(f'\n\n<msld_wrt> Writing out the initial ALF YAML file into {fyalf}')

        alfconfig = {'sysname' : sysname,\
                     'nsites' :  len(nsubs),\
                     'nblocks' : sum(nsubs),
                     'nsubs' : nsubs,
                     'nnodes' : 1,
        ...
        with open(fyalf, "w") as fyml:
            yaml.safe_dump(data = alfconfig, stream = fyml)
