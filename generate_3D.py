...
class RDKit_Tools:
    ...
    def generate_3D(self, mol_list="all", mol_names="all", mini=True, ref=None, writeSDF = False):
        """
        ...
        # Make sure molecule object and name lists are same length 
        try:
            assert (len(mol_names) == len(mol_list))
        except AssertionError:
            raise ValueError("List of names and list of molecules are not the same length.")
        ...
        if ref:
            if isinstance(ref,int):
                ...
                if ref.endswith(".sdf"):
                    ...
                    raise IOError('<generate_3D> (ref) must be a SDF file.')
